...
<%@ include file="sidebar.jsp" %>
 <article id="article">
   ...
   <table class="table1">
      <tbody>
         <tr class="tr1">
            <th class="th1"><input type="checkbox" id="cbx_chkAll" style="width:20px;"></th>
            <th class="th1" style="width:60px; min-width: 60px;">신청번호</th>
            <th class="th1">신청자</th>
            <th class="th1">부서</th>
            <th class="th1">직급</th>
            <th class="th1">신청일자</th>
            <th class="th1">근태일자</th>
            <th class="th1">근태구분</th>
            <th class="th1-content">신청사유</th>
      ...
      <c:forEach items="${attendList }" var="l" varStatus="status">
         <tr class="tr1">
            <td class="td1"><input type="checkbox" name="chk" id="chk" style="width:20px;"></td>
            ...
            <td class="td1"><c:if test="${l.egrade eq 0 }">사원</c:if>
            ...
            <td class="td1"><c:if test="${l.atregsts eq 0 }">병가</c:if>
            ...
            <td class="td1-content">${l.atregcontent }</td>
   ...
   <div class="page-div">
      ...
      <c:forEach var="i" begin="${ph.startPage}" end="${ph.endPage}">
         <a onclick="pageCss()" href="<c:url value='./attendRegister?page=${i}&pageSize=${ph.pageSize}'/>">${i}</a>
      </c:forEach>
   ...
   <!-- Modal -->
   <div class="modal" id="Modal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" data-backdrop="static">
    <!-- 모달 내용 -->
   ...
   <!-- Modal -->
   <div class="modal" id="editModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" data-bs-backdrop="static" data-keyboard="false">
    <!-- 모달 내용 -->
    ...
    <form action="./atedit" method="post" onsubmit="return editCheck()">
        <div class="modal-content">
         ...
         <div class="modal-footer edit-footer">
            <button type="submit" class="apBtn3">제출</button>
            <button type="button" class="closeBtn" data-bs-dismiss="modal">닫기</button>
         </div>
        </div>
    </form>
