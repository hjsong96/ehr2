	...
	function detail(apno){
		...
		$.ajax({
			...
			success:function(data){
				...
				//var containChk = $(stat+':contains('+myEname+')').text();
				/* if (containChk == null) {
					disableButton = true;
				} */
				...
				$("#aprealfile").html(data.aprealfile);
...
<%@ include file="sidebar.jsp" %>
 <article id="article">
<input type="hidden" value="${myInfo.myEno }" id="myEno">
<input type="hidden" value="${myInfo.myEname }" id="myEname">
<input type="hidden" value="${myInfo.myEgrade }" id="myEgrade">
<!-- 모달 start -->
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
			...
			<div id="apmemo" class="form-group">
			    ...
			    <div class="col-md-12" id="forAddApmemo" s>
			        <textarea id="apmemodetail" class="form-control apmemo" placeholder="Disabled input" disabled></textarea>
			    </div>
			</div>
			<!-- .apmemo 특이사항 end -->
			</div>
...
<div class="card">
	<div class="card-body">
		<h5 class="card-title">결재 하기</h5></div>
		    <div class="table-responsive">
		        <table class="table">
		            ...
		            <c:forEach items="${ListForDoAp}" var="ab">
		            <tbody class="customtable">
		                <tr class="aptable" onclick="detail(${ab.apno })">
		                    <th id="tapno">${ab.apno }</th>
		                    <td>${ab.aptitle }</td>
		                    <td class="">
								<c:if test="${ab.aplist eq 0}">경조금</c:if>
		                    	<c:if test="${ab.aplist eq 1}">교육신청</c:if>
		                    	<c:if test="${ab.aplist eq 2}">재직증명서</c:if>
		                    	<c:if test="${ab.aplist eq 3}">경력증명서</c:if>
		                    	<c:if test="${ab.aplist eq 4}">휴직</c:if>
		                    	<c:if test="${ab.aplist eq 5}">육아휴직</c:if>
		                    	<c:if test="${ab.aplist eq 6}">원천징수영수증</c:if></td>
		                    <td>
			                    <c:forEach items="${ApListForDoAp}" var="ap">
			                    	<c:if test="${ab.apno eq ap.apno }">
				                    	<c:if test="${ap.apacptdetail eq 0 }"><img height="20px" alt="대기" src="./img/yellow_icon.png"></c:if>
				                    	<c:if test="${ap.apacptdetail eq 1 }"><img height="20px" alt="승인" src="./img/green_icon.png"></c:if>
				                    	<c:if test="${ap.apacptdetail eq 2 }"><img height="20px" alt="반려" src="./img/red_icon.png"></c:if>
									</c:if>
			                    </c:forEach>
		                    </td>
		                    <td id="stat">
		                    	<c:set var="approvalStat" value="승인"></c:set>
		                    	<c:set var="stayStat" value="대기"></c:set>
		                    	<c:forEach items="${ApListForDoAp}" var="item">
